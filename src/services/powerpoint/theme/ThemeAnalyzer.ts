...
import { ColorScheme, FontScheme, FontSet, PlaceholderInfo, TextFormatInfo } from "../theme-types";
...
export class ThemeAnalyzer {
  ...
  public async analyzeColorScheme(context: PowerPoint.RequestContext): Promise<ColorScheme> {
    try {
      ...
      if (slides.items.length > 0) {
        const detectedColors = await this.extractColorsFromSlides(
          context,
          slides.items.slice(0, 3)
        );
        if (detectedColors) {
   ...
   */
  public checkColorAccessibility(
    foreground: string,
    background: string
  ): {
    ratio: number;
  ...
  public analyzeThemeConsistency(colorScheme: ColorScheme): {
    ...
    // アクセント色の調和チェック
    const accentColors = [colorScheme.accent1, colorScheme.accent2, colorScheme.accent3];
