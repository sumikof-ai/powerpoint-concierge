...
export class ThemeService {
  ...
  public selectOptimalLayout(
    ...
    return (
      optimalLayout || layouts.find((layout) => layout.type === "content") || layouts[0] || null
    );
  }
  ...
  public async detectThemeChanges(): Promise<boolean> {
    ...
    try {
      ...
      return (
        currentTheme.name !== this.cachedThemeInfo.name ||
        JSON.stringify(currentTheme.colorScheme) !==
          JSON.stringify(this.cachedThemeInfo.colorScheme)
      );
    } catch (error) {
