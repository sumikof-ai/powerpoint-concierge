...
/* global PowerPoint, console, File */
...
export class TemplateAdaptationService {
  ...
  async analyzeTemplateStructure(templateFile?: File): Promise<TemplateAnalysisResult> {
    try {
      if (templateFile) {
        // ファイルベースの分析（将来的な拡張）
        return await this.analyzeTemplateFile();
      } else {
        // 現在のプレゼンテーションの分析
   ...
   * 既存のアウトラインをテンプレートに適応
   */
  async adaptOutlineToTemplate(outline: any, template: TemplateInfo): Promise<AdaptedOutline> {
    try {
  ...
  private async analyzeCurrentPresentation(): Promise<TemplateAnalysisResult> {
    return await PowerPoint.run(async (context) => {
      ...
      const patterns = await this.extractDesignPatterns();
      const structure = await this.extractTemplateStructure();
  ...
  private async adaptSlideToTemplate(
    ...
    // スライドをテンプレートパターンに適応
    const relevantPatterns = template.designPatterns.filter(
      (p) => p.type === "layout" || p.type === "typography"
    );

    const adaptedContent = {
      title: slide.title || `スライド ${index + 1}`,
      content: slide.content || slide.keyPoints || [],
      slideType: this.mapSlideTypeToTemplate(slide.contentType),
      layoutSuggestion: this.suggestLayoutForSlide(slide),
  ...
  private mapSlideTypeToTemplate(contentType: string): string {
  ...
  private suggestLayoutForSlide(slide: any): string {
  ...
  private analyzeStructuralAdaptations(outline: any, template: TemplateInfo): TemplateAdaptation[] {
    const adaptations: TemplateAdaptation[] = [];
...
`;
  ...
  private async extractDesignPatterns(): Promise<DesignPattern[]> {
    const patterns: DesignPattern[] = [];
  ...
  private async extractTemplateStructure(): Promise<TemplateStructure> {
    return this.getDefaultTemplateStructure();
  }

  private async suggestMetadata(): Promise<Partial<TemplateMetadata>> {
    return {
  ...
  private buildTemplateInfo(analysisResult: TemplateAnalysisResult, themeInfo: any): TemplateInfo {
    return {
  ...
  private async analyzeTemplateFile(): Promise<TemplateAnalysisResult> {
    // ファイルベースの分析（将来的な実装）
    return {
      detectedPatterns: [],
      extractedStructure: this.getDefaultTemplateStructure(),
