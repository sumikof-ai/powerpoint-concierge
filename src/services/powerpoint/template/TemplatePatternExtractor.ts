...
export class TemplatePatternExtractor {
  ...
  async extractPatternsFromSlides(slides: PowerPoint.Slide[]): Promise<DesignPattern[]> {
    ...
    try {
      await PowerPoint.run(async (context) => {
        ...
        patterns.push(...(await this.extractSpacingPatterns()));
      });
    } catch (error) {
  ...
  async extractTemplateStructure(slides: PowerPoint.Slide[]): Promise<TemplateStructure> {
    try {
      ...
      const visualHierarchy = await this.analyzeVisualHierarchy();
  ...
  private async extractColorPatterns(slides: PowerPoint.Slide[]): Promise<DesignPattern[]> {
    ...
    for (const _slide of slides) {
      try {
        const colors = await this.extractColorsFromSlide();
  ...
  private async extractTypographyPatterns(slides: PowerPoint.Slide[]): Promise<DesignPattern[]> {
    ...
    for (const _slide of slides) {
      try {
        const typography = await this.extractTypographyFromSlide();
   ...
   * スペーシングパターンの抽出
   */
  private async extractSpacingPatterns(): Promise<DesignPattern[]> {
    const patterns: DesignPattern[] = [];
  ...
  private async analyzeSlideTypePatterns(slides: PowerPoint.Slide[]): Promise<SlideTypePattern[]> {
    ...
    for (let i = 0; i < slides.length; i++) {
      try {
        const slideType = await this.detectSlideType();
  ...
  private async analyzeContentFlow(slides: PowerPoint.Slide[]): Promise<ContentFlowPattern[]> {
    ...
    for (let i = 0; i < slides.length - 1; i++) {
      try {
        const currentType = await this.detectSlideType();
        const nextType = await this.detectSlideType();
  ...
  private async analyzeNavigationPattern(slides: PowerPoint.Slide[]): Promise<any> {
    ...
    for (const _slide of slides) {
      try {
        const content = await this.extractSlideTextContent();
   ...
   * 視覚的階層の分析
   */
  private async analyzeVisualHierarchy(): Promise<VisualHierarchyPattern[]> {
  ...
  private inferPresentationStyle(
    ...
    const hasNavigation =
      structure.navigationPattern.hasAgenda || structure.navigationPattern.hasTableOfContents;
  ...
  private inferTargetAudience(
    ...
    const hasComplexNavigation =
      structure.navigationPattern.hasTableOfContents && structure.navigationPattern.sectionDividers;
  ...
  private inferPurpose(
    ...
    const slideCount = structure.expectedSlideTypes.reduce(
      (sum, type) => sum + (type.frequency || 0),
      0
    );
  ...
  private async extractColorsFromSlide(): Promise<string[]> {
    // 実際の実装では、スライドから色情報を抽出
    return ["#000000", "#FFFFFF"]; // プレースホルダー
  }

  private async extractTypographyFromSlide(): Promise<{
    fonts: string[];
    sizes: number[];
  }> {
  ...
  private async detectSlideType(): Promise<string> {
    // 実際の実装では、スライドの内容を分析してタイプを判定
    return "content"; // プレースホルダー
  }

  private async extractSlideTextContent(): Promise<string> {
