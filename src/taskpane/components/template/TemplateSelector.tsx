...
const TemplateSelector: React.FC<TemplateSelectorProps> = ({
  ...
  const filterTemplates = () => {
    let templates = showRecommendationsOnly 
      ...
      >
        ...
        <div style={{ padding: '12px' }}>
          <div className={styles.templateTags}>
            <Badge color="brand" size="small">{template.category}</Badge>
            <Badge color="informative" size="small">{template.metadata.presentationStyle}</Badge>
            <Badge color="subtle" size="small">{template.metadata.targetAudience}</Badge>
          </div>

          <div className={styles.templateStats}>
            <Text size={100}>
              スライド数: {template.metadata.slideCount} | 
              用途: {template.metadata.purpose} | 
          ...
          {reasons.length > 0 && (
            <div className={styles.recommendationReasons}>
              <Text size={100} weight="medium">推奨理由:</Text>
    ...
    <div className={styles.container}>
      ...
      <div className={styles.filtersSection}>
        <Dropdown
          ...
          onOptionSelect={(_, data) => setSelectedCategory(data.optionValue as TemplateCategory | 'all')}
        >
